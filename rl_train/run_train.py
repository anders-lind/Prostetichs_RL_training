...
def a2c_evaluate_with_rendering(config):
    seed = 1234
    np.random.seed(seed)
    ...
    env.close()
def a2c_train_with_parameters(config, train_time_step, is_rendering_on, train_log_handler):
    seed = 1234
    np.random.seed(seed)
...
if __name__ == '__main__':
    ...
    if args.flag_realtime_evaluate:
        a2c_evaluate_with_rendering(config)
    else:
        a2c_train_with_parameters(config,
