...
class EnvironmentHandler:
    ...
    def get_stable_baselines3_model(config:TrainSessionConfigBase, env, trained_model_path:str|None=None):
        # 1. Import SAC instead of PPO
        import stable_baselines3
        
        # 2. --- CRITICAL ---
        # We can no longer use your custom policies (HumanActorCriticPolicy)
        # PPO uses an Actor-Critic policy.
        # SAC uses an Actor and Q-Value policies, which are a different architecture.
        # We will use the default SAC 'MlpPolicy'.
        policy_class = "MlpPolicy"
        print(f"Using SAC Algorithm with default '{policy_class}'")
        ...
        if trained_model_path is not None:
            ...
            # 4. Removed the 'model.policy.reset_network(...)' line
            # That function was specific to your custom PPO policy and does not exist
            # on the default SAC policy.
        else:
            ...
            model = stable_baselines3.SAC(
                ...
                env=env,
                verbose=2,
                # 5. Use sac_params and remove PPO-specific policy_kwargs
                **DictionableDataclass.to_dict(config.sac_params),
            )
        return model
    @staticmethod
